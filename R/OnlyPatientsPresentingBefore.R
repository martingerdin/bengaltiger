#' Only patients presenting within a specific delay
#'
#' Keeps only the patients whom presented within a specific delay, where delay
#' refers to the time between injury and arrival to hospital.
#'
...
#'     hours. Defaults to 6, i.e. only observations with an delay less than 6
#'     hours are kept in the sample.
...
#' @export
OnlyPatientsPresentingBefore <- function(study.sample, delay.variable.name = "delay", delay.cutoff = 6, remove.missing = TRUE) {
    ## Error handling
    ...
    exclusion.text <- paste0(total.n.excluded, " had a delay of ", delay.cutoff, " hours or more.")
    if (remove.missing) {
        exclusion.text <- paste0(total.n.excluded, " excluded: \n\n",
                                 "- ", n.missing, " had missing delay \n\n",
                                 "- ", n.excluded, " had a delay of ", delay.cutoff, " hours or more. \n\n")
    }
